// Logic Circuits.cpp: Okreœla punkt wejœcia dla aplikacji.
//
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_LOGICCIRCUITS, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_LOGICCIRCUITS));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_LOGICCIRCUITS));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_LOGICCIRCUITS);
...
char CWindow::Load(char*Path)
{
	...
	FILE*File = NULL;
	if ((File = fopen(Path, "rb")) == NULL) //otwarcie pliku do odczytu
	{
		delete Path;
		return -1;
	}
