...
CWindow::~CWindow()
{
	for(int i=0;i<NumOfElem;i++)
		delete TabElem[i];
	for(int i=0;i<=MenuButNum+6;i++)
		delete TabBut[i];
	if(StandardPath!=NULL)
		delete StandardPath;
	for(int i=0;i<ButNum-MenuButNum+1;i++) delete ImageTab[i];
}
...
void CWindow::React(int X,int Y)
{
	...
	if(NumOfElem>=MaxNumOfElem)
	{
		...
		Mode=block;
		for(int i=MenuButNum-1;i<ButNum;i++)
			TabBut[i]->SetActivity(false);
	}//zablokowanie klawiszy-za duzo element¢w
	else if(Mode==block)
	{
		Mode=move;
		for(int i=MenuButNum-1;i<ButNum;i++)
			TabBut[i]->SetActivity(true);
...
void CWindow::CheckElements(int X, int Y)	//cursor functionality
{
	POINT p;
	int x, y;
...
void CWindow::NewElement(int ElemNum)	//get mouse position
{
	//struct REGPACK reg;
	//reg.r_ax=0x2;
	...
	switch(ElemNum)
	{
		case MenuButNum:TabElem[NumOfElem]=new CAnd(x,y);
										ImageNum=0;break;
		case MenuButNum+1:TabElem[NumOfElem]=new COr(x,y);
											ImageNum=1;break;
		case MenuButNum+2:TabElem[NumOfElem]=new CNot(x,y);
											ImageNum=2;break;
		case MenuButNum+3:TabElem[NumOfElem]=new CNand(x,y);
											ImageNum=3;break;
		case MenuButNum+4:TabElem[NumOfElem]=new CNor(x,y);
											ImageNum=4;break;
		case MenuButNum+5:TabElem[NumOfElem]=new CXor(x,y);
											ImageNum=5;break;
		case MenuButNum+6:TabElem[NumOfElem]=new COutput(x,y);
											ImageNum=6;break;
...
char CWindow::DragElement(int ImageNum,int ElemNum)		//cursor
{
	//struct REGPACK reg;
	int X=-1;
	int Y=-1;
	...
	int XDif=TabElem[ElemNum]->GetXCorner()-x;
	int YDif=TabElem[ElemNum]->GetYCorner()-y;
	//reg.r_ax=0x4;
	//x=TabElem[ElemNum]->GetXCorner();
	//y=TabElem[ElemNum]->GetYCorner();
...
void CWindow::DrawWin(char Mode)	//handle colours
{
	...
	for(int i=7;i<10;i++)
		Text[i]='\0';
	DoText(WinNum,Text);
	TextOut(GetDC(GetActiveWindow()),300,10,(LPCWSTR)Text,wcslen(LPCWSTR(Text)));
	for(int i=0;i<ButNum;i++)
		TabBut[i]->DrawButton();
	for(int i=0;i<NumOfElem;i++)
		TabElem[i]->DrawElem();
...
void CWindow::DrawBasket(int X,int Y) //zbysiu pls add details
{
	/*setcolor(LineColour);
	line(X,Y,X,Y+30);
